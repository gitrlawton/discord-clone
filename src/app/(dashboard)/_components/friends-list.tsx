...
export function PendingFriendsList() {
  ...
  return (
    <div className="flex flex-col divide-y">
      ...
      {/** If the pending list is empty, render the empty component state. */}
      {friends?.length === 0 && (
        <FriendsListEmpty>No Pending Requests</FriendsListEmpty>
      )}

      {/** Otherwise, map over the friends and display the friend item for each one. */}
...
export function AcceptedFriendsList() {
  ...
  return (
    <div className="flex flex-col divide-y">
      ...
      {/** If the friends list is empty, render the empty component state. */}
      {friends?.length === 0 && (
        <FriendsListEmpty>
          Your friends will be displayed here.
        </FriendsListEmpty>
      )}
...
// Represents the empty state of a list.
...
// Component that generates a button with a tooltip.
function IconButton({
  title,
  className,
  icon,
...
// Component that represents a single friend in your friends pane.
function FriendItem({
  username,
  image,
  children,
